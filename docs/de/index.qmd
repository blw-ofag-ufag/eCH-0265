...
: Anzahl Direktzahlungskulturen (N) je Kulturkategorie (Kategorie) mit Referenzen auf die geltende Definition in der landwirtschaftlichen Begriffsverordnung (LBV), wo möglich. {#tbl-directpaymentcrops-categories}
...
Hier ist die Übersetzung, die für eCH-Standards (z. B. Konzept- oder Spezifikationsdokumente der Schweizer Behörden) optimiert wurde. Die direkte Anrede („Du/Sie“) wurde durch objektive, im Verwaltungskontext übliche Passivkonstruktionen ersetzt.

Wie gewünscht, wurde die Schweizer Rechtschreibung (ohne „ß“) angewendet.

---

# Nutzung der Daten

Das System der Kulturenstammdaten kann mittels der Abfragesprache [SPARQL 1.1 Query Language](https://www.w3.org/TR/sparql11-query/) abgefragt werden.
Nachfolgend ist ein [Abfragebeispiel](https://s.zazuko.com/3xUUXpv) aufgeführt, welches sämtliche URIs der Nutzungsarten sowie deren deutschsprachige Bezeichnungen abruft:
...
Weitere Beispiele sind im Verzeichnis [`src/sparql/queries`](https://www.google.com/search?q=src/sparql/queries) verfügbar. Es gilt zu beachten, dass bestimmte Beispiele parametrisiert sind und folglich ohne vorherige Modifikation nicht ausgeführt werden können.

Für den automatisierten Datenabruf aus LINDAS kann eine POST-Anfrage an den LINDAS-Endpunkt übermittelt werden, wobei die SPARQL-Abfrage als Parameter zu übergeben ist.

``` sh
# Beliebige Abfrage definieren...
...
Abhängig von den jeweiligen Integrationsanforderungen lässt sich der Accept-Header anpassen, um die Zieldaten in diversen Formaten zu beziehen. Dazu zählen unter anderem JSON (`application/json`), XML (`application/sparql-results+xml`) oder CSV (`text/csv`).
